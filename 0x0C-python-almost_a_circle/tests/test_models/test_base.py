...
class TestBase_load_from_file(unittest.TestCase):
    ...
    #     file_output = Base.load_from_file()
    #     self.assertIsInstance(file_output, list)
    ...
    def test_float(self):
        with self.assertRaises(TypeError):
            Base.load_from_file(4.99999)

    def test_complex(self):
        with self.assertRaises(TypeError):
            Base.load_from_file(complex(5))
    ...
    def test_tuple(self):
        with self.assertRaises(TypeError):
            Base.load_from_file(("4", 4))

    def test_set(self):
        with self.assertRaises(TypeError):
            Base.load_from_file({12, 24, 36})

    def test_frozenset(self):
        with self.assertRaises(TypeError):
            Base.load_from_file(frozenset({12, 24, 36}))

    def test_range(self):
        with self.assertRaises(TypeError):
            Base.load_from_file(range(5))

    def test_bytes(self):
        with self.assertRaises(TypeError):
            Base.load_from_file(b'AtoZ')

    def test_bytearray(self):
        with self.assertRaises(TypeError):
            Base.load_from_file(bytearray(b'AtoZ'))

    def test_memoryview(self):
        with self.assertRaises(TypeError):
            Base.load_from_file(memoryview(b'AtoZ'))

    def test_Inf(self):
        with self.assertRaises(TypeError):
            Base.load_from_file(float('Inf'))
